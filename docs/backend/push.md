...
A claim is a five-minute lease. If the function dies between claiming and settling, the row is due again when the lease runs out (and fails with `lease_expired` if that was its fifth attempt), so nothing is lost silently.
...
Run it from a real terminal (house rule 8: a non-interactive prompt can store an empty value). `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are provided by the platform. The **public** key also goes to the frontend as `VITE_VAPID_PUBLIC_KEY` (#704). A missing secret makes the function answer `500` naming it, **before** claiming anything, so no attempt is burned.
...
- **`failed` with `HTTP 401`/`403`**: the VAPID pair or subject is wrong or does not match the key the browser subscribed with. **`invalid_subscription`**: the stored token is not a usable `PushSubscription`.
