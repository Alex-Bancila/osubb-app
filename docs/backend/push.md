...
- **Self-repair at app start (#769).** Once per page load, after the service worker is ready, `usePushSelfRepair` (mounted in the signed-in shell) checks this device without asking anything: if push is on here — the Member's row for this browser's subscription exists, or they turned the switch on on this device (a per-Member flag in `localStorage`) — and the subscription was made with another key than `VITE_VAPID_PUBLIC_KEY`, or the row is missing, or the browser lost the subscription, it unsubscribes, subscribes again with the current key, stores the new row and only then deletes the stale one (if resubscribing fails, the row stays but the browser may be left without a subscription, so pushes to this device pause until a later start repairs it; the push-on flag makes sure it tries). So rotating the VAPID pair, or a `404`/`410` that removed a row, needs no action from Members beyond opening the app. One exception: the flag is written the first time a device opens a build with the self-repair while its row still exists, so a device subscribed before #769 whose row a `404`/`410` removed before that first visit reads as off, and its Member turns the switch on again once. The same holds wherever `localStorage` cannot keep the flag (some private windows): there the self-repair works only while the row exists. It never runs without a granted permission (it must not prompt), and it never adopts a subscription another Member left on a shared device.
...
- **Sign-out** deletes this device's row first (best-effort, bounded to five seconds), so a shared device stops receiving the previous Member's pushes. Sign-out and turning the switch off also clear the Member's push-on flag, and so does a session that ends any other way (expiry, another tab), so the self-repair leaves the device alone.
