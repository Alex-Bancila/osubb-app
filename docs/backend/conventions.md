...
Grants follow the shape: the `public` wrapper gets `revoke execute … from public, anon; grant execute … to authenticated`; the `_impl` function gets the same revoke plus `grant execute … to authenticated` (the invoker wrapper calls it as the caller, and `private` is never listed in `supabase/config.toml`'s `api.schemas`, so PostgREST cannot reach it directly). `require_*` helpers and trigger functions get no grant to `authenticated` at all — only the command that calls them needs to run them, and it runs as `security definer`.
...
- Policies: `task_read`, `event_read`, `ledger_read`, `ledger_sanction`, `request_*`, `assignee_*` (verb-first or domain-specific names instead of `<table>_<verb>`).
- Constraints: `projects_name_not_blank`, `projects_status_valid`, `projects_timestamps_ordered`, `project_members_role_valid` (no `_ck` suffix).
...
- [ ] Every new function has its own `revoke execute … from public, anon` (or `revoke all … from public, anon, authenticated` for triggers/`require_*` helpers).
