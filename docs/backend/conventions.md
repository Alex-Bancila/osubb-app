...
- [ ] Every new function revokes execute from `public, anon, authenticated, service_role`, then grants it back only to the roles that must call it (§4: `authenticated` for wrappers/`_impl`, no grant at all for `require_*`/trigger functions).
