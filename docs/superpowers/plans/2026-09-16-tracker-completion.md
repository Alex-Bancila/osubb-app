...
- `#482` reads `task_queue_summary`, which does not exist. Ruling: the queue position comes from `private.pending_candidate_count` / the member's own `task_candidates` row order — replace the view read with `select count(*) from task_candidates where task_id = $1 and status = 'pending' and joined_at < (own joined_at)` + 1, or ship the view in `#483`'s migration if that PR is where it was meant to come from; the implementer checks `#483`'s files first.
