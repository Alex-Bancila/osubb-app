...
## Execution rulings (2026-09-18 → 2026-09-19)

The durable record of every decision made while executing this plan. The SDD workspace (ledger, briefs, reports, verdicts) is git-ignored and deleted after the whole-wave review; what matters lives here.

**Outcome.** All six tasks merged: #160 (PR #503), #507 (PR #513), #508 (PR #514), #509 (PR #515), #510 (PR #516), #511 (PR #517), plus the closeout PR #518 (Refs #504). Issue graph applied from the reviewed dry run: milestones 17/18/19; umbrellas #504 (Wave 1), #505 (Wave 2), #506 (Wave 3); lane #507 → #508 → #509 → #510 → #511; #512 (Adunarea Generală eligibility, split from #66, Wave 3); 23 existing issues edited, #160 skipped as already closed. `push-staging` succeeded on every code merge, so the backfill, the mirror triggers and the `group_ids` hook are live on staging.

**Rulings, in order made.**

1. Coexistence is a shadow model with the legacy tables as write master (Alex, AskUserQuestion); the ADR's "compatibility views" sentence was corrected in #507.
2. Issue scope: Wave 1 in detail, Waves 2–3 as umbrellas; ADR-named issues edited (Alex).
3. BCE membership mirrors as `group_members.group_role = 'manager'`, re-derived on role change (Alex).
4. Three milestones, one per wave (Alex).
5. Plan-agent deviations accepted: partial sibling-name unique index (`groups_parent_name_uidx`, native Groups only until Wave 3 dedupes legacy names); `groups.created_by … on delete set null`; `service_role` select-only; auth-admin policies named per conventions §5. Open decisions: `group_ids` includes archived Groups; backfilled `application_level` pre-filled with `accepts_applications` false; Diverse/Secretariat `manager_title 'BCE'`; `roles.name` rename inside #507 as a second migration and commit.
6. Task 0 concerns: edit #160 only if still open (it was not); #66 split into a push_tokens issue (kept) and #512 (AG eligibility visibility, Wave 3, also blocks #48); #100 waits for #506; `max-1h` removed from #47 #48 #51 #52 #103 #105; twelve legacy-prefixed titles normalized to "Area: what".
7. #507 review: `groups_read` gained a third branch `or private.can_read_group_roster(id)` — a Group Manager or Responsible of an ancestor always sees the Group they hold authority over, even below its Minimum Level (ADR §Minimum Level sentence added). The `for no key update` race pin was deferred to #509. The claimless sweep cannot detect dropping `auth_is_member()` from a policy whose other branch already needs a live profile; `groups_schema.test.sql` carries a live-active-no-claims persona for that.
8. #508 review: `sync_team_groups()` self-heals Department Groups on its whole-table path; `sync_groups_from_legacy()` re-parents Teams before the orphan sweep (else `23503`). The leader-override proof needs a fixture created with `projects_sync_leader_membership` disabled, because that trigger otherwise guarantees the leader a `project_members` row.
9. #509 deviation accepted after a full lock analysis: the roster syncs' self-heal ensures a Group only when missing. An unconditional `on conflict … do update … where` still locks the conflicting row (`LockTupleNoKeyExclusive`, taken before the `WHERE` is evaluated) for the whole transaction and hung `select_task_candidate.test.sql` in a main-transaction ↔ dblink cycle the deadlock detector cannot see. The Group mirrors still run the full upsert for the changed row, so renames propagate. Race pin: `test_race` catches removing the parent lock; two `pgrowlocks` assertions catch removing it or swapping it for `for update`. Wave 3's Group commands must re-parent one row per statement (`cascade_group_path` cannot cope with a multi-row re-parent). The #508 orphan-sweep proofs were made honest again by disabling the mirror triggers around the fixture deletes.
10. #510: `group_ids` = explicit `group_members` rows only, numbers, ascending, archived Groups included; `auth_in_group()` uses jsonb containment so a forged string array fails closed; pins moved: `grants_hardening` 5→6 helpers, `rls_deny_by_default` auth-admin policies 4→6, `tracker_grants` public-function table +1; private roster ends at 107.
11. Whole-wave review B1, recorded not reverted: the shadow's read policies already implement the ADR's target visibility (every backfilled Group has `min_level 0`, rank BCE sees every roster), wider than `teams_read`/`projects_read`/`project_members_read`. Accepted because nothing exposed exceeds what the ADR grants, no screen reads the shadow yet, and production does not exist. S1: a closeout migration runs `private.sync_groups_from_legacy()` once after the triggers exist (the staging window between the #508 and #509 deploys). S2: "Campaign ownership by Group" belongs to Wave 2 (already in #505's scope). S3: `docs/README.md` gains ADR-0009.
12. Process: two implementers were killed by the harness watchdog (no output for 600 s) during `scripts/check-local-ci.sh`; both resumed from their transcripts with the work intact. Rule for every later dispatch: no single command over ~8 minutes; database gates run as separate bounded calls; `check-local-ci.sh` only per group.

**Deferred, with a home.** `is_interne` untouched until Wave 3 (#512); level 4 and the `responsabil` enum value until Wave 3 (BC re-ranks holders first — `seed.sql`, `shared_timestamps.test.sql` and `check-seed-rerunnable.sh` still insert one); the `org` pseudo-department row stays as the Organization Group's master until Wave 3; `= any (path)` is not GIN-indexable — Wave 2 probes ancestry with `path @> array[id]`; the Project mirror never re-syncs `created_by`/`created_at`.

**Staging checks for Alex.** Four queries (mirror completeness, drift/repair, login path, what every member can list) are in the whole-wave verdict and were handed over as `staging-checks.sql`.
