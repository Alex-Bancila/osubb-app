...
## Execution rulings (2026-09-19 → 2026-09-20)

The durable record of every decision made while executing this plan. The SDD workspace (ledger, briefs, reports, mutation tables) is git-ignored and deleted after the whole-wave review; what matters lives here.

**Outcome.** Task 0 filed the issue graph as planned: #519 (N1), #520 (N2), #521 (N3), #522 (N4), #523 (N7), #524 (N8) in milestone 18, #370 and #248 retitled onto Group-Role authority, 18 existing issues edited. Task 1 (#519) merged to `main` as PR #525 (c19b6d8) after three fix rounds. Then the plan met reality: dobrerares opened green, mergeable PRs overnight for every remaining issue — #549 (#520), #550 (#521), #552 (#522), #555 (#523), #564 (#370), #565 (#248), #566 (#524) — as a stack in that order. Alex ruled that this plan neither reviews nor merges those PRs and instead **stacks its deltas on top of them**. Tasks 2–8 therefore shipped as seven delta PRs, each cut from dobre's branch for that issue with our earlier deltas merged in, each targeting that branch and carrying no `Closes`: #567 (#520), #568 (#521), #569 (#522), #570 (#370), #571 (#523), #572 (#248), and the closeout delta on `docs/524-wave2-closeout`. Their implementations satisfied the briefs almost entirely; our deltas are dominated by assertions that mutation testing proved missing, plus two small migrations (#370's `PT400 event_group_required`, #248's five additions).

**Rulings, in order made.**

1. The auto-mode classifier refuses opaque scripts that write to GitHub; GitHub-visible batches run as explicit single `gh` commands (Task 0).
2. `create_event` keeps no legacy aliases: nothing in `app/` calls it, so #370 replaces the 10-arg definer outright and #506 drops only `create_completed_work_request`'s legacy parameters. #200 gets one `## Blocked by` section in place of its stale `**Dependencies**: None.` line. #248 stays on milestone 18.
3. Task 1 review: a spurious `campaign_group_origin_mismatch` on every non-Department Campaign, a caller-supplied `scope` silently rewritten on Event INSERT, and four non-disclosure comment blocks dropped from the re-issued Campaign impls — all invisible to a green suite. Round 1 fixed each and broke a neighbour; round 2 the same. Round 3 went to a fresh implementer on the most capable model with the semantics stated once: "the caller wrote X" is `new.X is not null` on INSERT and `new.X is distinct from old.X` on UPDATE, never `is not null` on UPDATE; both sides written means each written legacy value equals the named Group's own `legacy_*` column; the legacy-only arm exists on UPDATE too. `num_nonnulls(dept_id, team_id, project_id) > 1` with `group_id is not null` returns early in the Task/Request triggers so `tasks_exactly_one_origin_check` answers a malformed origin first (the reviewer's own `<> 1` was wrong — it aborts the first `group_id`-only INSERT); no such guard on `events`, whose invariant is a disjunction, documented instead. `group_id_for_legacy_origin` keeps `security definer` (an invoker-context caller would get `groups` RLS applied and resolve to null). `23514 <row>_group_required` for an unknown legacy id stays as specified, revisited in Wave 3. Two `*_unmapped` reasons are untestable until a native Group exists.
4. **Acceptance is by mutation, not by a green suite** — adopted after Task 1 and applied to every later task. Each function, and each ruling, must have a named assertion that fails when that one thing is reverted; where nothing fails, the assertion is written. Across Tasks 2–7 this found 34 shipped behaviours whose own suites did not notice their removal. Two survive by proof, not by gap: `require_task_evaluator` and `public.can_manage_tasks()` are equivalent to their legacy bodies while the shims route to the same Group — re-run both mutations at Wave 3's legacy drop.
5. `dblink`-based assertions (lock pins, `test_race`) cannot be mutation-tested in-transaction — the dblink session never sees an uncommitted `create or replace`. Mutate them on a committed scratch branch and `db reset` after, or state that they were proved by reading. psql's aligned output indents `not ok`; a mutation harness is validated against a known-failing mutant first and driven with `-q -t -A`.
6. No separate review seat for a mutation-proved, test-only delta of a few assertions; the mutation table is the proof.
7. The #318 timing tripwire ("~0.5 s") is a no-regression tripwire, not an absolute budget: a plain member's 5,000-Task read is ~533 ms against ~1359 ms for the pre-#521 body on the identical fixture, so the CTE hoist stays unbuilt.
8. Their `request_deciders` excludes the requester unconditionally where the brief scoped that to the Responsible branch. Kept: a Completed-work Request approved by its author is the conflict the rule exists to prevent. Consequence, pinned: a sole BC requester has no eligible decider.
9. Their test-file shapes differ from the briefs (new `group_commands` / `group_command_races` / `create_event_races` suites instead of rewriting the old ones in place); accepted, our assertions went into their files.
10. #370: the null-Group check sits last in the malformed block, not first, because nine of their claimless assertions pass a null Group beside another malformed argument and expect that argument's reason. The Moderator exemption from the upper Minimum Level bound is unfalsifiable (`min_level ∈ {0,3,5,6}`, Moderator is level 9) — kept and documented. `event_group_required` now exists as both `PT400` (command) and `23514` (trigger): same meaning at two layers, benign, to be collapsed when Wave 3 drops the trigger.
11. #523: the nearest-competing-ancestor proof drops `groups_competes_top_level_ck` inside its rolled-back transaction to build a two-competitor path — artificial by design; pinning the constraint alone would have left Wave 3 free to flip the ordering unseen.
12. #248: an Organization-Group _target_ applies `create_event`'s rule (level ≥ 6 or any live Group Role), because the root has no ancestor to reach it from; the source side keeps creator-or-level-6. Our step reorder means a caller with no authority over a move answers `42501` where #565 answered `PT409` — as specified; dobre is told rather than left to discover it.
13. Each of our branches tracks its own remote or none; three were found tracking dobre's branches, where a bare `git push` would have landed on theirs.
14. Whole-wave review (conditional pass; the five-row matrix walked persona by persona, every cell pinned; 9 of 9 re-run mutation kills reproduced): the `update_event` active-status check applies to the **target** Group only when the Group changes — an edit that keeps an Event in its archived Group is decided by the existing authority alone (BC always; a Group Role through `can_manage_group_work`'s own status gate); `reopen_task_impl` still deciding from `is_project_lead` was a Wave 2 goal violation, re-issued through the Wave 2 predicate in a closeout migration with identical signature and errors, and conventions §10 names the exact remaining legacy readers instead of "the two shims"; the Organization-_target_ rule for a moved Event stands, but its justification was wrong — the Organization Group is _a_ root with no ancestor, not the root of every path — and the rule is recorded in the dated ADR-0009 amendment, not only in a catalog comment; `PT400 event_group_required` joins the §3 error table beside its `23514` twin.
15. `public.can_manage_tasks()`'s legacy sweep survives mutation because of the `org` pseudo-department row, not because of path inheritance; the assertion that documents the survivor says so.
16. Fix round: `reopen_task_impl`'s Group decision cannot be told from its legacy one by any input reachable through the public command (`can_evaluate_task` judges the Assignment being reversed, and the mirror makes manager ≡ leader while Project Groups are roots), so its load-bearing assertion is a catalog one over `pg_get_functiondef` with the behavioural pair kept as coverage; `is_project_lead`, `is_project_responsible` and `can_manage_project_work` are left with no call site and recorded in conventions §10 as Wave 3 drops, not ports.

**Deferred, with a home.** Wave 3 (#506): the rolled-back `update public.groups set min_level / shared_work_visibility` fixtures move to the real Group command; three hard-coded `legacy_dept_id = 'org'` sites become the Organization setting; `cancel_event` gets a race suite; the `group_id is not null` limb of the Task/Request trigger guard and the events `v_caller_scope is not null` disjunct get their assertions when a command can reach them; `attendance_read`'s `auth_level() >= 4`; the two shims and the compat overloads; the `dept_cup` suite's weak coverage of the Cup body (1 of 10 mutants) is worth a rewrite when the Cup gets its frontend. `docs/agents/onboarding.md` (lines 48 and 141) still presents `private.can_manage_project_work` as a live security-definer policy helper; it has no call site after #524 and goes with the Wave 3 drop.

**Staging checks for Alex.** After the stack merges: the four Wave 1 queries, plus `select count(*) from public.tasks where group_id is null` = 0 and `select bool_and(private.group_id_for_legacy_origin(dept_id, team_id, project_id) = group_id) from public.tasks` = `t`.

**Handed to dobrerares, not acted on here (their code, outside this plan's authority).** ADR-0009's accepted text was edited in place in #566 rather than by dated amendment; the Organization branch of `create_event_impl`/`update_event_impl` locks every `group_members` row the actor holds as manager/responsible organization-wide `for share` (deadlock-safe, never a `groups` row, but far broader than the decision needs); `private.group_managers` relies on PL/pgSQL auto-declaring `v_depth`; `group_commands.test.sql` lacks `create extension if not exists pgtap`; `update_event_impl`/`cancel_event_impl` lock the target Event `for no key update` where conventions §2 says `for update` (probably the better choice, undocumented); `require_group_work_manager` evaluates `can_manage_group_work` up to three times per call under every Task command — the write path was never benchmarked.
