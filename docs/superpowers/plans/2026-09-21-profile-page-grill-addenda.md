...
- **#47 Evaluation Periods.** Add to *What to build*: "Closing a Period stamps `closing_threshold`: the Task
...
- **#512 AG eligibility visibility.** Replace the paragraph beginning "ADR-0009 §Promotion hooks defines what
  ...
  (#51)".
- **#594 Wave 4 umbrella.** In *Outcome*, replace "Voluntar Activ → Voluntar cu Drept de Vot is signalled to
  BC for human confirmation" with "a new Voluntar Activ is offered the adherence form and BC confirms Drept
  de Vot by hand; at close BC receives Retention Signals for both Roles". Add P3 to *Children*.
