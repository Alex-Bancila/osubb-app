...
- Migration header convention: first line `-- #<issue>: <one-line purpose>`; comments explain _why_.
...
- [ ] **Step 5: Reset and run everything** — `npx supabase db reset && npx supabase test db`. Expected: all suites green, `conventions.test.sql` `1..9`. If any _other_ suite now fails, the revoke broke a runtime path the precedent said was impossible — stop and report the failing assertion verbatim; do not paper over it.
