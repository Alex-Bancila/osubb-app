...
- **Branch protection on `main`:** Not available on the current free-plan private repo (see the note above) — until the plan changes, this records what to configure the day it becomes possible. Required status checks: `Migrations + db tests`, `Edge Function checks`, `Frontend checks`, and `Secret scan` (never `Push migrations to staging`, which runs only on `main`). GitHub does not let an author approve their own PR, so with a single maintainer, requiring one approving review would lock every PR out — require it once a second reviewer has landed, and until then allow administrators to bypass. Linear history optional. Enabling it is a human action in [repo settings](https://github.com/Alex-Bancila/osubb-app/settings/branches).
