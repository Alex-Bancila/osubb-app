...
The desktop and mobile screenshots render the production wrapper and form. A fixture supplies initial options, Playwright intercepts read requests, and the draft callback rejects with a synthetic `task_manage_forbidden` error with private details. The capture verifies one fresh `managed_work_groups` request before the callback, preserved title content, and absence of the private error text. No Task command or database write is involved.
