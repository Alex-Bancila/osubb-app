...
- **The frontier is the Tracker frontend, in dobrerares' open PRs.** Seventeen PRs (#471–#488) are open off `main`: the Tracker UI lane (#164–#175 — presentation model, table filtering, manager columns, member cards, role tabs, available work, express/withdraw interest, queue position, Task details, history, stage summary), the member-side Completed-work Request form (#351), and four backend follow-ups (#259 Department Cup totals, #260 leadership member history, #364 consolidating the live-actor helpers, #370 Event creation by owning scope). Run `gh pr list --state open` before assuming an issue behind one of these numbers has landed; its GitHub issue stays open until its PR merges.
- **Not built yet:** the Group model of ADR-0009 (three strangler waves, the next thing to plan); notification fan-out and the notification tables' RLS (#65, #68); the rest of the Tracker frontend beyond the open PRs (#176–#191, #346–#354); the shadcn/PWA foundation's remaining items (#322–#325); the rebuilt Calendar scope model; the Web Push provider; and Cloudflare production deployment. There are no transitional Task write paths left to extend — a direct client write to any Task table is a bug, not a shortcut.
- **Source of truth:** issue bodies and their numeric blockers define assignable work; a closed issue means merged to `main`, not merely "has an open PR". ADR-0007 (amended 2026-09-10) defines the target Tracker model; ADR-0008 defines Calendar behavior; ADR-0009 (2026-09-18) generalizes Departments, Teams, Projects, and the Adunarea Generală into Groups with settings and Group Roles, amends ADR-0004/0007/0008 by reference, and lands as a three-wave strangler — until it does, the code still speaks `dept_id / team_id / project_id`; ADR-0002/0005 define the browser/PWA architecture and rollout. `docs/backend/conventions.md` is binding on every migration — including the lock-mode rule (a parent/Umbrella row is locked `for no key update`, never `for update`) and the error-vocabulary rules the command wave settled. `docs/README.md` indexes every other document as authoritative or historical.
