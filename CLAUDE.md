...
- **Current backend queue:** the team lane is #276 → #277 → (#278 and #279) → #280 → #281. Task normalization follows #283–#296 plus the 2026-09-10 schema and command issues (campaigns, umbrella tasks, `unfulfilled`, evaluations, read policies, notification fan-out, completed-work requests), each using its explicit `## Blocked by` section. Startable now: #276, #283, #285, #286, #287, #256, #261, #65, #66 and the new unblocked reference-data, campaigns, and project-override issues.
- **Not built yet:** the normalized single-executor Task lifecycle, Candidate Queue and atomic Task commands; Campaigns, Umbrella Tasks, the `unfulfilled` outcome, Completed-work Requests, and notification fan-out (the notification tables also still lack RLS policies); shadcn/PWA migration; completed Task Tracker frontend; rebuilt Calendar scope model; Web Push provider; and Cloudflare production deployment. The legacy claim/open/multi-assignee paths are transitional and must not be extended.
