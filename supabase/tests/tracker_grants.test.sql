...
insert into expected_function_privs (proname, args, anon, auth_ex, svc, pub) values
  ...
  ('submit_task_for_review',  'p_task_id bigint, p_note text, p_link_label text, p_link_url text', false, true,  false, false),
  ...
  -- authenticated only -- even though set_group_role's manager branch is
  -- decided one level up, by the parent's Group Managers or by level >= 6.
  ('set_group_role',
  ...
  ('privacy_acknowledgement_status', '',                             false, true,  false, false);
...
insert into pinned_private_functions (proname, args, category) values
  ...
  ('submit_task_for_review_impl',                 'p_task_id bigint, p_note text, p_link_label text, p_link_url text',                                                  'impl'),
  ...
  -- #691: the Event mirror of validate_task_campaign; a trigger body, granted to nobody.
  ('validate_event_campaign',                     '',                                                                                                                   'trigger'),
  ...
  -- #583: the three Group roster command bodies, plus the shared Appointment
  -- core. The core carries no gate of its own -- every caller brings its own
  -- authority and #602's provisioning will too -- so, like
  -- cancel_event_effect, no client role may execute it.
  ('appoint_group_member',
   'p_group_id bigint, p_member_id uuid, p_actor uuid, p_group_role text, p_position_title text',
   'none'),
  ('set_group_role_impl',
   'p_group_id bigint, p_member_id uuid, p_group_role text, p_position_title text',
   'impl'),
  ...
  -- #675: the Nickname fold (the unique index's key, called as the caller by
  -- the invoker nickname guard, so authenticated keeps execute), the guard's
  -- trigger body, and the Member Card projection body behind public.member_card.
  ('fold_nickname',          'p_nickname text',  'authenticated_only'),
  ('guard_profile_nickname', '',                 'trigger'),
  ('member_card_impl',       'p_member_id uuid', 'impl'),
  ('group_coordination_impl', 'p_group_id bigint', 'impl'),
  -- #673: the constraints kit -- the step-1 length check every command calls
  -- from its security-definer body, the two pure helpers (called only by the
  -- security-definer row guards), and the two row-guard trigger bodies. None
  -- is granted to anybody.
  ('require_text_length',     'p_field text, p_value text, p_min integer, p_max integer', 'require'),
  ('is_http_url',             'p_value text', 'none'),
  ('normalize_phone',         'p_phone text', 'none'),
  ('guard_announcement_text', '',             'trigger'),
