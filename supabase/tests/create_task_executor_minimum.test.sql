...
select pg_temp.test_login_leadership(pg_temp.g521_uid(1));
select lives_ok($$select public.create_task('Eligible child #348', null, now()+interval '1 day', 'org', 'direct', p_executor_id => pg_temp.g521_uid(12), p_parent_task_id => (select id from g521_tasks where name='parent348'))$$,'a Member exactly at Minimum Level can execute a Subtask outside their memberships');
reset role;
...
select pg_temp.test_login_leadership(pg_temp.g521_uid(1));
select lives_ok($$select public.create_task('Unassigned child #348', null, now()+interval '1 day', 'org', 'direct', p_parent_task_id => (select id from g521_tasks where name='parent348'))$$,'a direct Subtask may still start without an Executor');
